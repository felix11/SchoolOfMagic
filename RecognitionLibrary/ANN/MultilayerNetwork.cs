...
namespace RecognitionLib.ANN
{
    ...
    class MultilayerNetwork
    {
        ...
        internal void backpropTraining(ArrayInputData traindata, int epochs)
        {
            ...
            Matrix pat = new DenseMatrix(traindata.Key.RowCount + 1, traindata.Key.ColumnCount, 1.0f);
            pat.SetSubMatrix(0, traindata.Key.RowCount, 0, traindata.Key.ColumnCount, traindata.Key);

            // target values init
            Matrix targets = traindata.Value;

            // dw, dv
            Matrix dw = new DenseMatrix(HiddenNodes, PatternSize + 1);
            Matrix dv = new DenseMatrix(TargetSize, HiddenNodes + 1);
        ...
        private void forwardPass(Matrix pat)
        {
            /*
                ...
                out = 2./ (1+exp(-oin)) - 1;
             */
            Matrix hin = weights1.Multiply(pat);
            ...
            Matrix oin = weights2.Multiply(net_hout);
        ...
        private void backwardPass(Matrix pat, Matrix targets)
        {
            /*
             ...
             */
            Matrix r1= (net_out.Add(new DenseMatrix(net_out.RowCount, net_out.ColumnCount, 1.0f))).PointwiseMultiply(new DenseMatrix(net_out.RowCount, net_out.ColumnCount, 1.0f).Subtract(net_out));
            net_deltao = (net_out - targets).PointwiseMultiply(r1).Multiply(0.5f);
            Matrix r2 = (net_hout.Add(new DenseMatrix(net_hout.RowCount, net_hout.ColumnCount, 1.0f))).PointwiseMultiply(new DenseMatrix(net_hout.RowCount, net_hout.ColumnCount, 1.0f).Subtract(net_hout));
        ...
        internal Matrix use(ArrayInputData pat)
        {
            // convert
